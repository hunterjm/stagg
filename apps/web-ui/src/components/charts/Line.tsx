...
export const LineChart = ({expanded=false, lines, labelType='ordinal', yStep=10, xStep=1}:Props) => {
  const options = {
    legend: {
      display: false,
      labels: {
  ...
  for(const line of lines) {
    if (data.labels.length < line.data.length) {
      data.labels = Object.keys(line.data).map(i => labelType === 'ordinal' ? ordinal(Number(i)+1) : '')
    }
    ...
    data.datasets.push({
      ...
      pointBorderColor: 'rgba(255, 255, 255, 0.33)',
      pointBackgroundColor: 'rgba(255, 255, 255, 0.33)',
      pointHoverBackgroundColor: 'rgba(255, 255, 255, 0.33)',
    })
    data.datasets.push({
      ...genericDataset,
      label: line.label,
  ...
  return <Line height={48} options={options} data={data} />
}
